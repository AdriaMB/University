...
int main(int argc, char **argv)
{
	...
	#pragma omp parallel
	{ 
	    long long private_hist[BINS];
	    #pragma omp for private(rx, ry, r2, r, inv_r3, ax, ay, v2, v_kms, bin) reduction(+:total_kinetic_energy) nowait
            for (i = 0; i < N; ++i) {
               rx = x[i]; 
	       ry = y[i];
               r2 = rx*rx + ry*ry;
    ...
    t2 = omp_get_wtime();
    printf("The total time of execution is %f\n ", t2-t1);
