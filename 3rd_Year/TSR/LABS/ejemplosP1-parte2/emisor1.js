...
function handler (event,n) { // function declaration, dynamic type args, higher-order function
 	return (x) => { // anonymous func, parameterless listener, closure
 		console.log(event + ":" + ++n + " times, and the value of x is: " + x)
 	}
}
...
console.log('---------------------------')
setInterval(()=>{emitter.emit(e1, 1000)}, 2000) // asynchronous (event loop), setInterval
setInterval(()=>{emitter.emit(e2, "some value")}, 8000) // asynchronous (event loop), setInterval
