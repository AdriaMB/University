...
void fase_COM_alum() {
    ...
    if (!terminando && RB[RB_inicio].ocupado &&
        ...
        if (inst_ecall(RB[RB_inicio].OP)) {
            ...
            // The register that is the destination of the oldest instruction of the ROB will be updated by the value of that ROB entry
            // RB_inicio is the identifier of the oldest entry of ROB
