...
#define I_OP codop
#define I_S1 rs1        // I_S1: the register source1 from the instruction under evaluation
#define I_S2 rs2        // I_S2: the register source2 from the instructino under evaluation
#define I_S3 rs3        // ...
#define I_D rd          // I_D: the destination register
#define I_INM imm       // the immediate value
#define I_RM rm         //
    ...
    if (I_PC == -1) {
        ...
        switch (opcode) {
            ...
            case OPCODE_OP_FP:
                ...
                /*** Operando 1 ***/
                if (Rfp[I_S1].rob == MARCA_NULA) {  // if the mark of the register FloatingPoint src1 is null...
                    // We can take directly the value. We will store the value in the Reservation Station
                    RS[s].V1 = Rfp[I_S1].valor;


                } else if (RB[Rfp[I_S1].rob].completado) { // there is a mark in the floatingPointBuffer, but the ROB entry whose mark can be found in Rfp states that the value can be taken
                    // We can take the value from the ROB
                    RS[s].V1 = RB[Rfp[I_S1].rob].valor;

                } else { // we cannot take the value. We must take the mark of the ROB that is modifying the value
