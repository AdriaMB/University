...
public class MunicipalityNetGraph {
    ...
    Map<Municipality, ListPOI<Municipality>> kruskalAdjacents; 
    ...
    public double createKruskalAdjacents() {
        ...
        double cost = 0; // total cost of the graph
        kruskalAdjacents = new HashTable<Municipality, ListPOI<Municipality>>(numVertices());
        
        for(int i = 0; i < edges.length; i++){
            Edge aux = edges[i];
            int srcVertice = aux.getSource(); // we get the vertice number
            Municipality src = verticesToMunicipalities.get(srcVertice); // src municiaplity
            
            int dstVertice = aux.getTarget();// we get the vertice number
            Municipality dst = verticesToMunicipalities.get(dstVertice); // dst municipality
            
            double w = aux.getWeight();
            
            if(kruskalAdjacents.get(src) == null){
                //We haven't init this node yet
                ListPOI<Municipality> adjacents = new LinkedListPOI<>();
                adjacents.add(dst);
                kruskalAdjacents.put(src, adjacents);
            }
            else{
                ListPOI<Municipality> adjacents = kruskalAdjacents.get(src);
                adjacents.add(dst);
                kruskalAdjacents.put(src, adjacents);
            }
            cost += w;
