...
public class BST<E extends Comparable<E>> {
    ...
    protected NodeBST<E> buildBalanced(E[] a, int begin, int end) {
        if(end-begin+1 == 0) return null;
        int mid = (end+begin)/2;
        NodeBST<E> left = buildBalanced(a, begin, mid-1);
        NodeBST<E> right = buildBalanced(a, mid+1, end);
        NodeBST<E> res = new NodeBST<E>(a[mid], left, right);
        return res;
    }

    /** Rebuilds the BST with the same data, such that it ends up balanced. */
    public void rebuildBalanced() {
       E[] theArray = toArrayInOrder();
       root = buildBalanced(theArray, 0, theArray.length-1);
    }
