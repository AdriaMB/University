...
public class Resident implements Comparable<Resident>{
    ...
    public boolean equals(Object other) {
        if(other instanceof Resident){
            Resident evil = (Resident)other;
            return this.getDni().compareTo(evil.getDni()) == 0;
        }
        return false;
    }
     ...
     */
    public int compareTo(Resident evil) {
        int aux = this.getSurname1().compareTo(evil.getSurname1());
        if(aux != 0){ return aux;}
        else{
            aux = this.getSurname2().compareTo(evil.getSurname2());
            if(aux != 0){ return aux;}
            else{
                aux = this.getName().compareTo(evil.getName());
                if(aux != 0){ return aux;}
                else{
                   aux = this.getDni().compareTo(evil.getDni()); 
                   return aux;
                }
            }
        }
