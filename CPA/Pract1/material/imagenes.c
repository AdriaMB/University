           ...
           int n, int m)
{
  ...
  for (p = 0; p < pasos; p++) {
    //printf("The number of threads in p = %d: %d\n", p, omp_get_num_threads()); 
    //printf(" I am thread %d\n", omp_get_thread_num());
    #pragma omp parallel for private(j,k, l, v, tot, resultado)
	
    for (i = 0; i < n; i++) {
     // printf(" I am thread %d and my i = %d\n", omp_get_thread_num(), i);

      for (j = 0; j < m; j++) {
//	printf(" I am thread %d and my j = %d\n", omp_get_thread_num(), j);
...
//	printf("HI from thread %d and j = %d\n", omp_get_thread_num(), j);
  ...
  t4 = omp_get_wtime();
  printf("Enlapsed time in filter method= %f\n", t4-t3);
...
int main()
{
  ...
  printf("The total number of current active threads BEFORE: %d\n", omp_get_num_threads());
